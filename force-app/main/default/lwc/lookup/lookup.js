...
import getOneRecordById from '@salesforce/apex/LookupServiceController.getOneRecordById'
import getRecent from '@salesforce/apex/LookupServiceController.getRecent'
import getRecords from '@salesforce/apex/LookupServiceController.getRecords'
...
export default class Lookup extends LightningElement {
  ...
  @api recordId; // Gets the current record ID
  @api objectApiName; // Gets the object API name
  ...
  connectedCallback () {
    ...
    console.log('Immediately:', this.objectApiName);
    setTimeout(() => console.log('After timeout:', this.objectApiName), 1000);
    if (this.value) {
      this.requestOneById()
    } else {
      this.requestRecent()
  ...
  clickRecord(event) {
    console.log('Clicked Record Event:', event.target);

    // Traverse up to find the div that contains data attributes
    let targetElement = event.target.closest('div[role="option"]');

    if (targetElement) {
        let recordId = targetElement.dataset.recordId;
        let recordName = targetElement.dataset.recordName;
        
        console.log('Selected Record ID:', recordId);
        console.log('Selected Record Name:', recordName);
        this.inputValue = recordName;
    }
}
  ...
  selectItem () {
    ...
    const listbox = this.template.querySelector('c-list-box')
    listbox.selectItem()
  }
